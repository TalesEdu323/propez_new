...
import { FileText, Info, Copy, Type, Hash, Calendar, User, Building2 } from 'lucide-react';

interface Placeholder {
  key: string;
  label: string;
  icon: React.ReactNode;
  group: 'empresa' | 'cliente' | 'geral';
}

const PLACEHOLDERS: Placeholder[] = [
  { key: '{{EMPRESA_NOME}}', label: 'Nome da sua empresa', icon: <Building2 className="w-3 h-3" />, group: 'empresa' },
  { key: '{{EMPRESA_CNPJ}}', label: 'CNPJ da sua empresa', icon: <Hash className="w-3 h-3" />, group: 'empresa' },
  { key: '{{CLIENTE_NOME}}', label: 'Nome do Cliente', icon: <User className="w-3 h-3" />, group: 'cliente' },
  { key: '{{CLIENTE_EMPRESA}}', label: 'Empresa do Cliente', icon: <Type className="w-3 h-3" />, group: 'cliente' },
  { key: '{{VALOR_TOTAL}}', label: 'Valor Total', icon: <Hash className="w-3 h-3" />, group: 'geral' },
  { key: '{{DATA_ATUAL}}', label: 'Data de Hoje', icon: <Calendar className="w-3 h-3" />, group: 'geral' },
  { key: '{{SERVICOS_LISTA}}', label: 'Lista de Serviços', icon: <FileText className="w-3 h-3" />, group: 'geral' },
];

const GROUP_LABELS: Record<Placeholder['group'], string> = {
  empresa: 'Sua empresa',
  cliente: 'Cliente',
  geral: 'Proposta',
};
...
function PlaceholderGroup({
  ...
  return (
    <div className="space-y-2">
      <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest px-1">{GROUP_LABELS[group]}</p>
      {items.map((p) => (
        <button
          key={p.key}
          type="button"
...
export default function ContractEditor({ value, onChange }: ContractEditorProps) {
  ...
  const byGroup = (group: Placeholder['group']) => PLACEHOLDERS.filter((p) => p.group === group);
      ...
      <div className="w-full lg:w-64 flex flex-col gap-4">
        <div className="bg-zinc-50 rounded-2xl p-4 border border-black/5">
          ...
          <p className="text-[11px] text-zinc-500 mb-4 leading-relaxed">
            Clique nos campos abaixo para inserir no contrato. Nome e CNPJ da sua empresa vêm de Configurações.
          </p>
          <div className="space-y-4">
            <PlaceholderGroup group="empresa" items={byGroup('empresa')} onInsert={insertAtCursor} />
            <PlaceholderGroup group="cliente" items={byGroup('cliente')} onInsert={insertAtCursor} />
            <PlaceholderGroup group="geral" items={byGroup('geral')} onInsert={insertAtCursor} />
          </div>
        </div>
        ...
        <div className="bg-black text-white rounded-2xl p-4 shadow-lg">
          ...
          <p className="text-[11px] leading-relaxed opacity-90">
            Use os campos da seção &quot;Sua empresa&quot; na qualificação da contratada; nome e CNPJ são preenchidos automaticamente nas propostas.
          </p>
        </div>
      </div>
...
CONTRATANTE: {{CLIENTE_NOME}}, {{CLIENTE_EMPRESA}}
